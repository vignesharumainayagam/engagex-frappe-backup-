...
$.extend(frappe.desktop, {
   ...
   dash_chart :function(){
    ...
    lead_report :function(){
    	frappe.call({
        method: 'frappe.desk.doctype.desktop_icon.desktop_icon.total_lead',
        callback: (response) => {
            if (response) {
                console.log(response);
                frappe.desktop.wrapper.find('.leadreportdash').append(frappe.render_template('lead_report_',response.message));
    ...
    function lead_chart() {
        let data = {
            labels: ["Open", "Converted", "Lost", "Closed"],
        ...
        let chart = new Chart({
            parent: "#chartt", // or a DOM element
            title: "Opportunity by Status",
            data: data,
    ...
    function repor_char() {
        let bar_composite_chart = new Chart({
            ...
            title: "Monthly Based Lead Report",
            data: {
                labels: ["Jan", "Feb", "Mar", "Apr",
                ...
                datasets: [{
                    title: "Leads",
                    values: [25, 40, 30, 35, 8, 52, 17, 14, 45, 23, 55, 23]
                }]
            },
            type: 'bar',
            height: 180,
