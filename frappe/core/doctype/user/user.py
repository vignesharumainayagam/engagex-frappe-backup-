...
@frappe.whitelist(allow_guest=True)
def student_sign_up(email, full_name, redirect_to, mobile_no, new_password):
	if not is_signup_enabled():
		frappe.throw(_('Sign Up is disabled'), title='Not Allowed')
	...
	if user:
		...
		if user.flags.email_sent:
			...
			return 2, _("You have sucessfully signed up!!! You may login using your credentials")		
