		...
		if "run_method" in frappe.local.form_dict:
			...
			if name:
				...
				if frappe.local.request.method=="GET":
					...
					frappe.local.response.update({
						"data":  frappe.call(frappe.client.get_listapi,
							doctype, status, **frappe.local.form_dict)})
				...
				if frappe.local.request.method=="POST":
					...
					frappe.local.response.update({
						"data": frappe.get_doc(data).insertapi(status).as_dict()
					})
					frappe.db.commit()
			else:
				raise frappe.DoesNotExistError
...
def vaidate_key():
	...
	key = frappe.get_request_header("token")
	akey = frappe.db.get_value('User', user, 'key')
