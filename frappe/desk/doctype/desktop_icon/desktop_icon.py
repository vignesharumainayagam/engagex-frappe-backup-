...
def total_count():
	return {
		...
		"rec_pay"	: frappe.db.get_all('Payment Entry', fields=['name','party_name','received_amount','creation'], filters={'docstatus': 1},limit=6),
		"stu_list"	: frappe.db.get_all('Student', fields=['name','title','creation'],limit=6),
		"instructor": frappe.db.get_all('Instructor', fields=['instructor_name','department','creation','employee'],limit=6),
		"task"	: frappe.db.get_all('Daily Task', fields=['name','subject','expected_start_date','expected_end_date','status'],limit=6)
	}
@frappe.whitelist()	
def total_lead():
	return {
		"Status"		: frappe.db.sql('select status, count(status) from `tabOpportunity`  group by status'),
		"Source"		: frappe.db.sql('select source, count(source) from `tabLead`  group by source'),
		"monthly"		: frappe.db.sql('select "January" as Month,count(name) from `tabStudent` where MONTH(joining_date)=1 UNION select "February" as Month,count(name) from `tabStudent` where MONTH(joining_date)=2 UNION select "March" as Month,count(name) from `tabStudent`where MONTH(joining_date)=3 UNION select "April" as Month,count(name) from `tabStudent` where MONTH(joining_date)=4 UNION select "May" as Month,count(name) from `tabStudent`where MONTH(joining_date)=5 UNION select "June" as Month,count(name) from `tabStudent`where MONTH(joining_date)=6 UNION select "July" as Month,count(name) from `tabStudent` where MONTH(joining_date)=7 UNION select "August" as Month,count(name) from `tabStudent`where MONTH(joining_date)=8 UNION select "September" as Month,count(name) from `tabStudent`where MONTH(joining_date)=9 UNION select "October" as Month,count(name) from `tabStudent` where MONTH(joining_date)=10 UNION select "November" as Month,count(name) from `tabStudent` where MONTH(joining_date)=11 UNION select "December" as Month,count(name) from `tabStudent`where MONTH(joining_date)=12'),
		"task"			: frappe.db.get_all('Lead', fields=['lead_name','contact_number','email_id','source','status'],limit_page_length=14)
